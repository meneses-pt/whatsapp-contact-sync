 ...
 * pulled ~100 photos per run before every remaining contact looked photo-less.
 * Pacing the requests and retrying a throttle with exponential backoff lets far
 * more contacts through before the run finishes.
 */
export async function downloadFile(
  ...
  for (let attempt = 0; attempt <= MAX_PROFILE_PIC_RETRIES; attempt++) {
    ...
    if (result.status === "throttled" && attempt < MAX_PROFILE_PIC_RETRIES) {
      ...
      console.log(`[sync] profile-pic throttled for ${whatsappId} (code=${result.code}) — backing off ${backoff}ms`);
      await sleep(backoff);
      continue;
    }
    if (result.status === "throttled") {
      console.log(`[sync] profile-pic still throttled for ${whatsappId} after ${MAX_PROFILE_PIC_RETRIES} retries — skipping`);
    }
